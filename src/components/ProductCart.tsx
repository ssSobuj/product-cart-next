  ...
  return (
    <div className="rounded-lg w-full flex flex-col md:flex-row gap-x-[3.75rem] py-10">
      ...
      <div className="md:w-1/2 w-full mt-5 md:mt-0 flex flex-col justify-center">
        ...
        {/* Color Selection */}
        <div className="mb-6">
          <h3 className="text-[#364A63] font-bold mb-[0.625rem]">Band Color</h3>
            ...
            {productDetails.colors.map((color) => (
              <button
                ...
                className={`w-4 aspect-square rounded-full ${
                  color.name === selectedColor ? "ring-2 ring-offset-2" : ""
                }`}
                style={{
        ...
        <div className="mb-6">
          ...
          <div className="flex gap-2">
            {Object.entries(productDetails.priceBySize).map(([size, price]) => (
              <button
                key={size}
                className={`px-[1.125rem] text-[#364A63] text-sm py-2 border rounded-md ${
                  size === selectedSize ? "border-[#6576FF]" : "border-gray-300"
                }`}
                data-size={size}
              ...
              >
                <span
                  className={`font-bold ${
                    size === selectedSize ? "text-[#6576FF]" : "text-[#364A63]"
                  }`}
                >
                  {size}
                </span>
        ...
        <div className="flex gap-x-4">
          <div className="flex items-center border rounded-md">
            ...
            <span className="px-[1.625rem] py-2 border-x">{cartQuantity}</span>
            <button
          ...
          <button className="bg-[#6576FF] text-white px-[1.125rem] py-2 rounded hover:bg-[#7C3AED]">
            Add to Cart
          </button>
        </div>
      </div>
